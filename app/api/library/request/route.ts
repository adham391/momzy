    ...
    after(async () => {
      try {
        ...
        if (account?.password_hash) {
          ...
          await sendEmail({
            to: account.email,
            subject: libraryEmailSubject("reset"),
            html: libraryEmailHtml({ setupUrl: `${siteUrl}/library/setup/${raw}`, purpose: "reset" }),
          });
          ...
          await sendEmail({
            to: acc.email,
            subject: libraryEmailSubject("setup"),
            html: libraryEmailHtml({ setupUrl: `${siteUrl}/library/setup/${raw}`, purpose: "setup" }),
          });
        }
