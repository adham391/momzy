...
const COPY: Record<BookingState, { title: string; subtitle: string }> = {
  confirmed: {
    title: "تم تسجيلكِ بنجاح 🌸",
    subtitle: "مقعدكِ محجوز — بانتظاركِ!",
  },
  awaiting: {
    title: "حجزنا مقعدكِ — بانتظار الدفع",
    subtitle: "أكملي الدفع بالأسفل لتثبيت مقعدكِ",
  },
  cancelled: {
    title: "التسجيل ملغى",
...
function StateIcon({ state }: { state: BookingState }) {
  if (state === "confirmed") {
    return <Image src="/icons/correct-icon.png" alt="تم" width={48} height={48} className="object-contain" />;
  }
  if (state === "awaiting") {
    return (
  ...
  return (
    <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="var(--mid)" strokeWidth="2" strokeLinecap="round" aria-label="ملغى">
...
export default function BookingHeader({ bookingNumber, createdAt, state }: BookingHeaderProps) {
  ...
  return (
    ...
    >
      ...
      >
        <span className="font-label text-[13px] text-mid">رقم التسجيل:</span>
