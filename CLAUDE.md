...
| `/admin/orders` + `/[id]` | فلاتر + بحث + تغيير حالة (+سجل) + رقم تتبّع + ملاحظات داخلية + **طباعة שטר מטען** (`/[id]/waybill`): ملصق صغير بعرض 100 مم في أعلى A4، ملصق لكل وجهة (هدية فيزيائية ← مستلِمتها)، عناوين عبرية مع نقحرة للعربي، بلا أسعار ولا توقيع — للطلب المدفوع الذي فيه ما يُشحن (`lib/orders/waybill.ts`) |
